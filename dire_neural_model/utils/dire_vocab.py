...
class VocabEntry:
    def __init__(self, subtoken_model_file_path=None):
        self.word2id = dict()
        self.subtoken_model_file_name = subtoken_model_file_path
        if subtoken_model_file_path:
            self.subtoken_model = spm.SentencePieceProcessor()
            self.subtoken_model.load(subtoken_model_file_path)
            vocab_path = subtoken_model_file_path[: subtoken_model_file_path.rfind('.model')] + '.vocab'
    ...
    def eos_id(self):
        return self.subtoken_model.eos_id()

    def bos_id(self):
        return self.subtoken_model.bos_id()

    def __getitem__(self, word):
        return self.word2id.get(word, self.unk_id)

    def vectorized_encode_as_subtoken_ids(self, word: str):
        encode_as_ids = self.subtoken_model.encode_as_ids(word)
        if self.vocab_size is not None:  # case of functions - where we knwo what the size of the vocab is and we want to embed
            one_hot = np.zeros((self.vocab_size, 1))
            one_hot[encode_as_ids] = 1
            return one_hot
        else:
            eos_id = self.eos_id()
            return encode_as_ids + eos_id

    def encode_as_subtoken_ids_list(self, word: str, alpha=100):
        # encode_as_ids = self.subtoken_model.encode(word, alpha=alpha)
        encode_as_ids = self.subtoken_model.encode_as_ids(word)
        eos_id = self.eos_id()
        return encode_as_ids + [eos_id]
    ...
    @property
    def params(self):
        subtoken_model_file_name = os.path.basename(
            self.subtoken_model_file_name) if self.subtoken_model_file_name is not None else None
        params = dict(unk_id=self.unk_id, word2id=self.word2id,
                      subtoken_model_file_name=subtoken_model_file_name)
    ...
    @classmethod
    def load(cls, path=None, vocab_files_folder=None, params=None, vocab_size=None):
        if path:
            params = json.load(open(path, 'r'))
        else:
            assert params, 'Params must be given when path is None!'

        if 'subtoken_model_path' in params and params['subtoken_model_path'] is not None:
            subtoken_model_path = os.path.join(vocab_files_folder, params['subtoken_model_path'])
            entry = VocabEntry(subtoken_model_path)

        else:
            entry = VocabEntry(None)
        # somehow, what we get is not good for here:! and the following lines ruined all
    ...
    def load(cls, path):
        ...
        for key, val in params.items():
            if key in ('grammar',):
                ...
                entry = VocabEntry.load(vocab_files_folder=os.path.dirname(path), params=val)
