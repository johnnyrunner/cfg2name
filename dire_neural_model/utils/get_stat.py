...
def compute_dataset_stat():
    ...
    for dataset_id, dataset in enumerate([train_set, dev_set, test_set]):
        for example in dataset.get_iterator(num_workers=5):
            example_num_variables = len(example._variable_name_map)

            n_var_new_name = 0
            for old_var_name, new_var_name in example._variable_name_map.items():
            ...
            ast_sizes.append(example._ast.size)
