...
from dire_neural_model.utils.ast import SyntaxNode
from dire_neural_model.utils.code_processing import canonicalize_code, preprocess_ast, tokenize_raw_code

from dire_neural_model.utils.dataset import json_line_reader, Example
...
def is_valid_example(example):
    try:
        is_valid = example._ast.size < 300 and \
...
def generate_dire_example(tree_json_dict):
    try:
...
def example_generator(json_queue, example_queue, args, consumer_num=1):
    ...
    while True:
        ...
        for json_str, meta in payload:
            ...
            if is_valid:
                canonical_code = canonicalize_code(example._ast.code)
                example.canonical_code = canonical_code
            ...
            if examples:
                ...
                with open(os.path.join(tgt_folder, 'files/', json_file_name), 'w') as f:
                    for example in examples:
                        ...
                        all_functions.setdefault(json_file_name, dict())[example._ast.compilation_unit] = example.canonical_code
