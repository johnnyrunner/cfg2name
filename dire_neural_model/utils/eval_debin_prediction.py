# evaluate debin
import json
from .dataset import get_json_iterator_from_tar_file, Example, Dataset
...
def evaluate_debin(debin_output_path, test_example_path):
    ...
    for example in debin_predicted_data.get_iterator(num_workers=5):
        example_key = example.binary_file['file_name'].split('/')[-1] + '_' + str(example._ast.compilation_unit)

        assert example_key not in debin_prediction_dict

        pred_result = dict(example._variable_name_map.items())
    ...
    for example in test_set.get_iterator(num_workers=5):
        ...
        if example_key in debin_prediction_dict:
            ...
            rename_result = {k: k for k in example._variable_name_map}  # use identity predictions
