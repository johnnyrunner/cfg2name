...
from .dire_vocab import SAME_VARIABLE_TOKEN, DireVocab
import random
...
## this is ubuntu stuff i commented out
# import resource
...
class Batcher(object):
    def __init__(self, config, train=True):
        ...
        if not self.is_ensemble:
            self.vocab = DireVocab.load(config['data']['vocab_file'])
        ...
        self.init_gnn_with_seq_encoding = config['encoder']['type'] == 'GraphASTEncoder' and config['encoder'][
    ...
    def annotate_example(self, example) -> Example:
        ...
        tgt_pred_seq_len = 0
        for old_name, new_name in example._variable_name_map.items():
    ...
    def to_tensor_dict(self, examples: List[Example], return_prediction_target=True) -> Dict[str, torch.Tensor]:
        ...
        if self.config['encoder']['type'] == 'GraphASTEncoder':
            ...
            packed_graph, tensor_dict = GraphASTEncoder.to_packed_graph([e.ast for e in examples],
                                                                        connections=self.config['encoder'][
                                                                            'connections'],
            ...
            packed_graph, gnn_tensor_dict = GraphASTEncoder.to_packed_graph([e.ast for e in examples],
                                                                            connections=
                                                                            self.config['encoder']['graph_encoder'][
                                                                                'connections'])
    ...
    def to_batched_prediction_target(self, examples: List[Example]):
        ...
        asts = [e.ast for e in examples]
        var_name_maps = [e.variable_name_map for e in examples]
        examples_tensors_dict = self.extract_batch_examples_tensors(batch_size, asts, var_name_maps,
                                                                    unchanged_var_weight, variable_name_subtoken_maps)
        return examples_tensors_dict

    @staticmethod
    def extract_batch_examples_tensors(batch_size, asts, var_name_maps, unchanged_var_weight,
                                       variable_name_subtoken_maps, method_names_subtokens):
        ...
        examples_tensors_dict = dict(variable_tgt_name_id=variable_tgt_name_id,
                                     ...
                                     method_tgt_name_ids=method_tgt_name_ids)
        return examples_tensors_dict
...
def is_valid_training_example(example):
    ...
    return example._ast.size < 300 and \
           len(example._variable_name_map) > 0 and \
           any(k != v for k, v in example._variable_name_map.items())
...
def example_to_batch(json_queue, batched_examples_queue, batch_size, train, config, worker_manager_lock,
                     return_examples=False, return_prediction_target=True):
...
class Dataset(object):
    ...
    def _get_iterator(self, shuffle=False, num_workers=1):
        ...
        json_loader = multiprocessing.Process(target=json_line_reader,
                                              args=(self.file_paths, json_enc_queue, num_workers,
    ...
    def batch_iterator(self, batch_size: int, config: Dict,
                       ...
                       num_readers=3,
                       num_batchers=3,
                       progress=True, train=False, single_batcher=False) -> Iterable[
        Union[Batch, Dict[str, torch.Tensor]]]:
        if progress:
    ...
    def _batch_iterator(self, batch_size: int, config: Dict, num_readers, num_batchers, train=False,
        ...
        for i in range(num_readers):
            p = multiprocessing.Process(target=example_to_batch,
                                        args=(json_enc_queue, worker_result_queue, batch_size, train, config,
                                              worker_manager_lock, return_examples, return_prediction_target))
            p.daemon = True
            example_generators.append(p)
        ...
        worker_manager_thread = threading.Thread(target=worker_manager, args=(
            worker_result_queue, batch_queue, num_readers, worker_manager_lock, 100))
    ...
    def _single_process_batch_iter(self, batch_size: int, config: Dict, num_readers=2, shuffle=False) -> Iterable[
        Batch]:
        ...
        from dire_neural_model.utils.preprocess import is_valid_example
        for example in filter(is_valid_example, example_iter):
            batch_examples.append(example)
            batch_node_num += example._ast.size
...
if __name__ == '__main__':
    for _example in Dataset('data/0-trees.tar.gz'):
        if _example._ast.size > 200:
            print(_example.binary_file, _example._variable_name_map)
