...
from dire_neural_model.model.embedding import NodeTypeEmbedder, SubTokenEmbedder
from dire_neural_model.model.encoder import Encoder
from dire_neural_model.model.gnn import GatedGraphNeuralNetwork, AdjacencyList
from dire_neural_model.model.sequential_encoder import SequentialEncoder
from dire_neural_model.utils import util
...
from dire_neural_model.utils.dire_vocab import DireVocab, ROUTINIO_DIR
...
class GraphASTEncoder(Encoder):
    ...
    def __init__(self,
        ...
        #
        # self.var_node_name_embedding = nn.Embedding(874, gnn.hidden_size, padding_idx=0)
    ...
    def build(cls, config):
        ...
        node_content_embedder = SubTokenEmbedder(vocab.obj_name.subtoken_model_file_name, params['node_content_embedding_size'])
    ...
    def forward(self, tensor_dict: Dict[str, torch.Tensor]):
        tensor_dict = tensor_dict[0] #TODO: check how exactly
        ...
        if env_vars.use_gpu:
            tree_node_init_encoding = tree_node_init_encoding.cuda()
    ...
    def to_packed_graph(cls, asts: List[AbstractSyntaxTree], connections: List,
        ...
        for ast_id, ast in enumerate(asts):
            ...
            for i, (var_name, var_nodes) in enumerate(ast.variables.items()):
                ...
                if 'func_root_to_arg' in connections and 'is_arg' in dir(var_nodes[0]) and var_nodes[0].is_arg:
    ...
    def to_tensor_dict(cls, packed_graph: PackedGraph,
                       grammar: Grammar,
                       vocab: DireVocab) -> Dict[str, torch.Tensor]:
        ...
        node_type_tokens_list = [[] for _ in range(packed_graph.size)]
        ...
        for i, (ast_id, group, node_key) in enumerate(packed_graph.nodes.values()):
            try:
                ...
                print('did not succeed loading function to be a tensor')
