...
from ..utils.dire_vocab import DireVocab, SAME_VARIABLE_TOKEN, END_OF_VARIABLE_TOKEN

from ..model.recurrent_subtoken_decoder import RecurrentSubtokenDecoder
from ..utils.dire_vocab import DireVocab
...
class AttentionalRecurrentSubtokenDecoder(RecurrentSubtokenDecoder):
    def __init__(self, variable_encoding_size: int, context_encoding_size: int, hidden_size: int, dropout: float,
                 tie_embed: bool, input_feed: bool, vocab: DireVocab):
        super(AttentionalRecurrentSubtokenDecoder, self).__init__(variable_encoding_size, hidden_size, dropout,
                                                                  tie_embed, input_feed, vocab)
    ...
    def build(cls, config):
        ...
        vocab = DireVocab.load(params['vocab_file'])
    ...
    def predict_tensors(self, examples_batch: Batch, encoder: Encoder) -> List[Any]:
        # for FunctionNamer Kavitzky
        # asts_variables = [example.ast.variables for example in examples]
        asts_variables = examples_batch['asts_variables'][0]['var_names']
        batch_size = len(asts_variables)
        ...
        variable_nums = []
        for ast_variables in asts_variables:
            variable_nums.append(len(ast_variables))
        ...
        # tensor_dict = self.batcher.to_tensor_dict(examples_batch)
        tensor_dict = dict([i for i in examples_batch])
        # tensor_dict = examples_batch
        ...
        for t in range(0, max_prediction_time_step):
            ...
            for beam_id, (ast_id, beam) in enumerate(beams.items()):
                ...
                for i in range(len(beam_new_hyp_positions)):
                    ...
                    # TODO: make sure worked
                    prev_hyp = beam[int(prev_hyp_id)]
                    ...
                    if hyp_var_name_id == end_of_variable_id:
                        ...
                        if remove_duplicate:
                            ...
                            if any(x == last_pred for x in new_variable_list[:-1] if
                                   x != [same_variable_id, end_of_variable_id]):
            ...
            if live_beam_ids:
                ...
                if self.independent_prediction_for_each_variable:
                    is_same_variable_mask = torch.tensor(is_same_variable_mask, device=self.device,
        ...
        for i, hyps in enumerate(completed_hyps):
            variables = asts_variables[i]
            hyps = sorted(hyps, key=lambda hyp: -hyp.score)

            if not hyps:
                # return identity renamings
                print(f'Failed to found a hypothesis for function', file=sys.stderr)
                ...
                for hyp in hyps:
                    variable_rename_result = dict()
                    for var_id, old_name in enumerate(variables):
